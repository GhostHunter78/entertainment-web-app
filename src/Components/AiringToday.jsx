  ...
  const getAiringToday = async () => {
    try {
      const response = await fetch(
        "https://api.themoviedb.org/3/tv/airing_today?api_key=399464050f3fe0a143819e2d3fa3f29b"
      );
      if (!response.ok) {
        throw new Error(
          `Error fetching airing today series: ${response.status}`
        );
      }
    ...
    <div className="mt-6 w-screen px-4">
      <div className="flex flex-row items-center w-full justify-between">
        <div className="flex items-center gap-[8px]">
          <p className="outfit text-white text-[20px]">Airing Today</p>
