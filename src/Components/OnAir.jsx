...
function OnAir({ currentPathName, currentPage, movieOrTv }) {
  const [onAirData, setOnAirData] = useState(null);

  const getOnAir = async () => {
    try {
      const response = await fetch(
        "https://api.themoviedb.org/3/tv/on_the_air?api_key=399464050f3fe0a143819e2d3fa3f29b"
      );
      if (!response.ok) {
        throw new Error(`Error fetching onAir series: ${response.status}`);
      }
      const data = await response.json();
      setOnAirData(data.results);
    } catch (error) {
      console.error("Error fetching onAir series:", error);
    }
  };
    ...
    <div className="mt-6 w-screen px-4">
      <div className="flex flex-row items-center w-full justify-between">
        <div className="flex items-center gap-[8px]">
          <p className="outfit text-white text-[20px]">On The Air</p>
